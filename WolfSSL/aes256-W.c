...
int Decrypta (FILE * fInput, FILE * fOutput) 
{
    ...
    wolfSSL_EVP_CipherInit_ex (&ctx, wolfSSL_EVP_aes_256_cbc (), NULL, k.data, iv.data, 0); 
...
int Encrypta (FILE * fInput, FILE * fOutput) 
{
    ...
    wolfSSL_EVP_CipherInit_ex (&ctx, wolfSSL_EVP_aes_256_cbc (), NULL, k.data, iv.data, 1);  
...
int main (int argc, char *argv[]) 
{
	...
	{
        printf ("AES256-W - Open input plain text file %s error (rc = %d).\n", argv[1], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("AES256-W - Open output encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("AES256-W - Open input encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}

    if ( (rc = OpenFile (&fDecrypt, argv[3], "w")) )
	{
        printf ("AES256-W - Open output decrypt text file %s error (rc = %d).\n", argv[3], rc);
		return rc;
	}
