...
int main (int argc, char *argv[]) 
{
	...
	{
        printf ("GCM256-B - Open input plain text file %s error (rc = %d).\n", argv[1], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("GCM256-B - Open output encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("GCM256-B - Open input encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}

    if ( (rc = OpenFile (&fDecrypt, argv[3], "w")) )
	{
        printf ("GCM256-B - Open output decrypt text file %s error (rc = %d).\n", argv[3], rc);
		return rc;
	}
