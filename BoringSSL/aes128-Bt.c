...
int Decrypta (FILE * fInput, FILE * fOutput) 
{
    ...
    EVP_DecryptInit_ex (&ctx, EVP_aes_128_cbc (), NULL, k.data, iv.data);
...
int Encrypta (FILE * fInput, FILE * fOutput) 
{
    ...
    EVP_CIPHER_CTX ctx;
    EVP_CIPHER_CTX_init (&ctx);
    EVP_EncryptInit_ex (&ctx, EVP_aes_128_cbc (), NULL, k.data, iv.data);
...
int main (int argc, char *argv[]) 
{
	...
	{
        printf ("AES128-B - Open input plain text file %s error (rc = %d).\n", argv[1], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("AES128-B - Open output encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("AES128-B - Open input encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}

    if ( (rc = OpenFile (&fDecrypt, argv[3], "w")) )
	{
        printf ("AES128-B - Open output decrypt text file %s error (rc = %d).\n", argv[3], rc);
		return rc;
	}
