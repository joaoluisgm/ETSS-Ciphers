...
int main (int argc, char *argv[]) 
{
	...
	{
        printf ("AES256-L - Open input plain text file %s error (rc = %d).\n", argv[1], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("AES256-L - Open output encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}
    ...
    if ( fEncrypt == NULL )
	{
        printf ("AES256-L - Open input encrypt text file %s error (rc = %d).\n", argv[2], rc);
		return rc;
	}

    if ( (rc = OpenFile (&fDecrypt, argv[3], "w")) )
	{
        printf ("AES256-L - Open output decrypt text file %s error (rc = %d).\n", argv[3], rc);
		return rc;
	}
